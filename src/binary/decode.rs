...
impl<R: std::io::Read> ByteReader<R> {
    ...
    pub fn decode<D>(&mut self) -> anyhow::Result<D>
    where
        D: Decode<R>,
        D::Tag: Decode<R, Tag = ()>,
    {
        let tag = D::Tag::decode(self, ())?;
        self.decode_with_tag(tag)
    }

    pub fn decode_with_tag<D>(&mut self, tag: D::Tag) -> anyhow::Result<D>
    where
        D: Decode<R>,
        D::Tag: Decode<R, Tag = ()>,
    {
...
impl<R: std::io::Read, T: DecodeTag> Decode<R> for T {
    ...
    fn decode(bytes: &mut ByteReader<R>, _: Self::Tag) -> anyhow::Result<Self> {
        ...
        let byte = bytes.next()?;
        Self::decode_tag(byte).with_context(|| {
            format!(
                "unexpected byte for `{}`: 0x{byte:0>2X} at offset {offset:0>8X}",
